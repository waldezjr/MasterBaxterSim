...
def circular_traj(t,x_c,tSim):
    
    omega = 2*pi/tSim
    r = 0.05 #radius 
    if t<tSim:
        x_ref = np.matrix([x_c[0]+r*cos(omega*t),x_c[1]+r*sin(omega*t),x_c[2]])
        x_ref_dot = np.matrix([-r*omega*sin(omega*t),r*omega*cos(omega*t),0.0])
        x_ref_dot_dot = np.matrix([-r*omega*omega*cos(omega*t),-r*omega*omega*sin(omega*t),0.0])
    else:
...
def main():
    ...
    t = 0
    t_ros_cur = -1.0
    x_c_right = [0.65,-0.3,0.1]
    x_c_left = [0.65,0.3,0.1]
    ...
    while not rospy.is_shutdown():
        ...
        x_ref_right,x_ref_dot_right,x_ref_dot_dot_right = circular_traj(t,x_c_right,30)
        x_ref_left,x_ref_dot_left,x_ref_dot_dot_left = circular_traj(t,x_c_left,30)


        right_arm_ctrl.run(x_ref_right,x_ref_dot_right,orient_ref,False)
        left_arm_ctrl.run(x_ref_left,x_ref_dot_left,orient_ref,False)
        rate.sleep()
